...
import com.rupeelog.data.local.database.InsightDao
import com.rupeelog.data.local.preferences.EncryptedPreferences
import com.rupeelog.agents.reporting.ReportingAgent
        ...
        routing {
            ...
            get("/") {
                call.respond(ApiResponse(
                    success = true,
                    message = "RupeeLog Dashboard API - MCP Secured",
                    data = mapOf(
                        "version" to "1.1.0",
...
// Extension functions
private fun com.rupeelog.data.local.database.TransactionEntity.toApiModel() = TransactionApi(
    id = id,
    amount = amount,
    currency = currency,
...
private fun com.rupeelog.data.local.database.InsightEntity.toApiModel() = InsightApi(
    id = id,
    type = type,
    title = title,
...
private fun com.rupeelog.data.local.database.BudgetEntity.toApiModel() = BudgetApi(
    id = id,
    category = category,
    amount = amount,
